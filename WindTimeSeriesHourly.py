...
def windTimeSerieshourly(ftp, rootFolder, selectedDate, time, outputFolder, ymax):
    ...
    for item in horas_str:
        ...
        time_folder2 = '/home/perfilador/data/'+utc_day+'/'+dataType+'/'+utc_hour+'-00/'
        creat_min = []
        ftp.dir(time_folder, creat_min.append)
        #dirs=os.scandir(time_folder2)
        #print('create min:', creat_min)
        #print('scan:')
        #with os.scandir(time_folder2) as dirs:
            #for entry in dirs:
                #print(entry.name)
        #print('-'*30)
    ...
    for working_file in file_list:
        ...
        obj = DataContainer()
        #obj.datastr=ifile.read()
        ftp.retrbinary('RETR %s' % working_file, obj.readFromFTP)
        #print('*'*5,'working file:', working_file, match)
    ...
    W=df_W.values
    # Vertical wind grid
    #print('m_time:', meas_time)
    ranges=np.linspace(100,2500,49)
    nx, ny = (49,(ranges)) # 49 is
    #x = np.linspace(0,len(meas_time),nx)
    
    x = np.linspace(0,len(meas_time),48)#48 o 49??
    y = np.linspace(min(ranges),max(ranges),49)
    X, Y = np.meshgrid(x, y)
